...
if __name__ == "__main__":
    start_date = "20180101"
    end_date = "20200824"
    test_data_path = utils.Base_DIR + "/0030"
    test_data = DataManager.load_data(test_data_path, date_start=start_date, date_end=end_date)
    test_data = DataManager.get_moving_trend(test_data, w=20, v=7)
    # test_data = DataManager.get_ulbv_vector(test_data)
    ...
    model_path = utils.SAVE_DIR + "/Models" + "/MLPFeatureExtractor.pth"
    ...
    Visualizer.get_action_and_candle(test_data.iloc[:400], samples["action"].iloc[:400])
    # # metrics.reset()
